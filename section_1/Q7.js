...
function solution(day, carNumbers) {
  let carList = carNumbers.filter(x => x % 10 === day);

  return console.log(carList.length);
}
...
/* [풀이]
  두 자리의 차량 번호를 10 나눈 나머지가 차량번호의 일의 자리임을 이용
  주어진 차량 번호 리스트에서 filter 함수를 사용하여 일의 자리와 day와 같은 차량을 선별
  선별된 차량 배열의 length 값 리턴
*/
